  ...
  if(length(subnet)>0){
   ...
   if(n_distinct(gg$data[gg$data$label==gene_of_interest,]$diagnosis)>1){
    ...
    ggsave(paste0(file.path(basepath,saveplots), gene_of_interest, '.png'),
           device = 'png', dpi = 720,height = 10,  width = 16)
...
# Fig S5
```{r}
# Note: Figure 4 and S5 are made manually via Cytoscape 3.8.0
...
```{r}
rm(basepath)
save.image(file.path('tmp/multiomics.RData'))
