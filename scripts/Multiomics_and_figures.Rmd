...
studycolors = c('CD' = '#c25572',
                'UC' = '#ff9706',
                'nonIBD' = '#00a1d5ff',
                'ChEMBL'="#374e55ff",
                'Enrichment analysis'='#f4e218',
                'GWAS'='#fb8072',
                'Negative' = 'firebrick4',
                'Positive' = 'steelblue', 
                'other' = 'black')
...
HMP2_metabs <- 
  readxl::read_xlsx(file.path(basepath,'elaborated_data/2018-01-01078D-s6/Supplementary Tables 1-28.xlsx'), 
                    sheet=2) %>%
...
mets.hist <- pri.met %>% 
  ...
  theme_minimal()+
  scale_fill_tableau('Color Blind', na.value='bisque3')+
...
met.alluv <-  pri.met %>% 
  ...
  alluvial_wide(stratum_label_size = 3,
                ...
                col_vector_flow = c(tableau_color_pal('Color Blind')(length(unique(pri.met$targetclass))-1), 
                                    'grey'), 
                stratum_labels = F, 
...
bioMAP_plot <- readxl::read_xlsx(file.path(basepath, 'elaborated_data/BioMap_results.xlsx'),sheet = 1) %>% 
  ...
  filter(system_name_required %in% c('BT', 'CASM3C','HDF3CGF'), 
         metabolite %in% c('Butyric acid', 'Niacin', 'alpha-CEHC', 'Oleanolic acid')) %>% 
  ...
  theme_minimal()+
  geom_ribbon(aes(ymin=-significance_prediction_envelope_at_95_percent, 
                  ymax=significance_prediction_envelope_at_95_percent), 
              group=1, alpha=0.2) +
  geom_line(aes(color=conc_levels, group=concentration_molar), alpha=.8) +
  scale_colour_gradient2_tableau('Green-Blue Diverging',
                                 name='Metabolite\nconcentration\n(log mM)')+
...
file.copy(file.path(basepath, 'elaborated_data/BioMap_results.xlsx'), 
          file.path(basepath, 'results/manuscript_files/Supplementary_Table7.xlsx'))
```
...
xgb.cm <- cm$table %>% 
  ...
  geom_tile()+
  theme_minimal()+
  scale_fill_gradient(low='white',high='#003c67ff')+
  xlab("True")+
  ylab("Predicted")+
...
up.htx.df <- htx_results %>% 
  ...
  ungroup %>% 
  full_join(readxl::read_xlsx(file.path(basepath,'elaborated_data/2018-01-01078D-s6/Supplementary Tables 31-33.xlsx'), sheet = 1) %>% 
...
bioMAP_plot <- readxl::read_xlsx(file.path(basepath, 'elaborated_data/BioMap_results.xlsx'),sheet = 1) %>% 
  ...
  arrange(concentration_molar) %>% 
  mutate(conc_levels = log1p(1000*as.numeric(concentration_molar))) %>% 
  filter(system_name_required != 'HPNo') %>% 
  ungroup %>% 
  arrange(metabolite, conc_levels, bio_marker_name_s_required) %>% 
  mutate(metabolite = str_replace(metabolite, 'Butyric acid', 'Butyrate'),
         metabolite = str_replace(metabolite, 'Niacin', 'Nicotinic acid')) %>% 
  ggplot()+
  aes(y=log_ratio_required, x=bio_marker_name_s_required)+
  facet_grid(metabolite~system_name_required, 
             scales = 'free', 
             space = 'free_x')+
  theme_minimal()+
  geom_ribbon(aes(ymin=-significance_prediction_envelope_at_95_percent, 
              ...
              group=1, alpha=0.2) +
  geom_line(aes(color=conc_levels, group=concentration_molar), alpha=.8) +
  scale_color_gradient2_tableau('Green-Blue Diverging', 
                                name='Metabolite\nconcentration\n(log mM)')+
...
bioMAP_plot
ggsave(file.path(basepath,'results/manuscript_files/FigS6.pdf'), device = 'pdf', width = 25, height = 18, units = 'in')
```
...
HMP2_important_metabs <- 
  readxl::read_xlsx('elaborated_data/2018-01-01078D-s6/Supplementary Tables 1-28.xlsx', sheet=16) %>%
...
```{r Sup3}
HMP2_important_htx <- 
  readxl::read_xlsx(file.path(basepath,'elaborated_data/2018-01-01078D-s6/Supplementary Tables 31-33.xlsx'), sheet = 1) %>% 
  group_by(Comparison) %>% 
...
Sup3 %>% 
  ungroup %>% 
  left_join(select(t_classes, Targets = hgncid, targetclass)) %>% 
  type_convert() %>% 
...
Sup4 %>% 
  ...
  mutate(Target = case_when(hgncid %in% Sup3$Targets ~ paste0(hgncid,footnote_marker_number('§', "html")),
                            TRUE~ hgncid), 
         variant = str_replace_all(variant,'_', ' '), 
         `Direction of modulation` = str_split(candidates, ' ', simplify=T)[1], 
         `Metabolite abundance` = case_when(MWU_delta > 0 ~ 'Enriched', 
                                            MWU_delta < 0 ~ 'Depleted')) %>% 
  cSplit('variant',';','long') %>% 
  select(Target, 
         `Genetic association` = `DISEASE/TRAIT`,
         Variant = variant, 
         `GWAS p-value` = `P-VALUE`,
         PUBMEDID, 
         `Additional pathway evidence` = action, 
         `∆expr` = log2FoldChange,
         qval = padj, 
         `Enriched pathways` = Pathway, 
         diagnosis, 
         Metabolites, 
...
```{r}
openxlsx::read.xlsx(file.path(basepath,'results/manuscript_files/Supplementary_Table6.xlsx')) %>% 
  group_by(Metabolites, diagnosis) %>% 
  filter(n_distinct(sign(Metabolite.differential.concentration)) == 1) %>% 
  ungroup %>% 
  # filter(Metabolite.scoring>quantile(Metabolite.scoring,.75)) %>% 
  filter(diagnosis=='CD') %>% 
  group_by(Metabolites, Target) %>% 
  summarize(`Score CD` = max(Metabolite.scoring), 
            Modulation = paste0(unique(Modulation), collapse='; '),
            `highest pxC50` = max(pxc50),
            `Metabolite abundance CD`= unique(sign(Metabolite.differential.concentration)), 
            `Metabolite abundance CD` = case_when(`Metabolite abundance CD`>0 ~'Enriched',
                                                  `Metabolite abundance CD`<0 ~'Depleted'),
            `Target expression CD`= unique(Target.differential.expression), 
            `Target expression CD` = case_when(`Target expression CD`>0 ~'Enriched', 
                                                `Target expression CD`<0 ~'Depleted')) %>% 
  full_join(openxlsx::read.xlsx(file.path(basepath,'results/manuscript_files/Supplementary_Table6.xlsx')) %>% 
              group_by(Metabolites, diagnosis) %>% 
              filter(n_distinct(sign(Metabolite.differential.concentration)) == 1) %>% 
              ungroup %>% 
              # filter(Metabolite.scoring>quantile(Metabolite.scoring,.75)) %>% 
              filter(diagnosis=='UC') %>% 
              group_by(Metabolites, Target) %>% 
              summarize(`Score UC` = max(Metabolite.scoring), 
                        Modulation = paste0(unique(Modulation), collapse='; '),
                        `highest pxC50` = max(pxc50),
                        `Metabolite abundance UC`= unique(sign(Metabolite.differential.concentration)), 
                        `Metabolite abundance UC` = case_when(`Metabolite abundance UC`>0 ~'Enriched',
                                                              `Metabolite abundance UC`<0 ~'Depleted'),
                        `Target expression UC`= unique(Target.differential.expression), 
                        `Target expression UC` = case_when(`Target expression UC`>0 ~'Enriched', 
                                                           `Target expression UC`<0 ~'Depleted'))) %>% 
  left_join(openxlsx::read.xlsx(file.path(basepath,'results/manuscript_files/Supplementary_Table6.xlsx')) %>% 
              group_by(Metabolites, diagnosis) %>% 
              filter(n_distinct(sign(Metabolite.differential.concentration)) == 1) %>% 
              ungroup %>% 
              # filter(Metabolite.scoring>quantile(Metabolite.scoring,.75)) %>% 
              group_by(Metabolites, Target) %>% 
              summarize(analogs = paste0(unique(analog), collapse='; '))) %>% 
  select(Metabolites, analogs, `highest pxC50`, Modulation, Target, everything()) %>% 
  filter(Modulation != 'other')  %>% 
  distinct_all  %>% 
  openxlsx::write.xlsx(file.path(basepath,'results/manuscript_files/Supplementary_Table5.xlsx'))
```
