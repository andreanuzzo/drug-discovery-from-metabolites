...
```{r}
project_folder = "~/projects/HMP2_Share"
knitr::opts_knit$set(root.dir=project_folder, dpi=720)
knitr::opts_chunk$set(root.dir = project_folder, dpi=720)
...
metadata <- fread(file.path(project_folder,'elaborated_data/HMP2_sample_data')) %>%
              clean_names() %>%
...
col_order = intersect(colnames(cts), rownames(metadata))
...
dds <- DESeqDataSetFromMatrix(countData = cts, colData = metadata, 
                              design = ~ diagnosis)
...
ese <- ExpressionSet(assay(se),AnnotatedDataFrame(metadata))
...
data.frame(x = tsne_out$Y[,1], 
            y = tsne_out$Y[,2], 
            col = metadata[colnames(cts),]$biopsy_location, 
            size = metadata[colnames(cts),]$consent_age,
            shape = metadata[colnames(cts),]$diagnosis) %>% 
  ggplot() + 
...
limmads <- limma::removeBatchEffect(x=assay(dds),
    batch = metadata[rownames(t(assay(dds))),]$biopsy_location)
...
data.frame(x = limtsne_out$Y[,1], 
            y = limtsne_out$Y[,2], 
            diagnosis = metadata[colnames(cts),]$diagnosis, 
           col = metadata[colnames(cts),]$biopsy_location, 
            size = metadata[colnames(cts),]$consent_age,
            shape = metadata[colnames(cts),]$diagnosis) %>% 
  ggplot() + 
...
# DESeq2
```{r, warning=F}
# colData(dds) = cbind(metadata[colnames(dat),], as.data.frame(svseq$sv) %>% clean_names)
# svs = as.data.frame(svseq$sv)
# colnames(svs)
# colData(dds) = cbind(DataFrame(metadata), DataFrame(svs))
...
data.frame(x = rmtsne_out$Y[,1], 
            y = rmtsne_out$Y[,2], 
            diagnosis = metadata[colnames(dat),]$diagnosis,
            biopsy_location = metadata[colnames(dat),]$biopsy_location) %>% 
  ggplot() + 
...
consensus <- duplicateCorrelation(normcts, 
                                  design,
                                  block = metadata[colnames(counts(dds)),]$biopsy_location)
...
data.frame(x = vrmtsne_out$Y[,1], 
            y = vrmtsne_out$Y[,2], 
            diagnosis = metadata[rownames(t(assay(dds))),]$diagnosis,
            biopsy_location = metadata[rownames(t(assay(dds))),]$biopsy_location) %>% 
  ggplot() + 
  geom_point(aes(x=x, y=y, shape=diagnosis, color=biopsy_location), size = 2.5)+ 
  theme_minimal()+
  ggthemes::scale_color_pander()
...
UC_RA_clu <- cluster_pathways(UC_RAout, plot_hmap = TRUE, )
...
CD_RA_clu <- cluster_pathways(CD_RAout, plot_hmap = TRUE)
