...
```{r, warning=FALSE}
#
techreps <- fread('raw_data/HMP2_metabolomics.csv.gz') %>% 
  colnames %>% 
  `[`(grepl('techrep|TR',.)) %>% 
  str_remove_all('_techrep|_TR')

metabolomics_raw <- fread('raw_data/HMP2_metabolomics.csv.gz')
sample_data <- fread('elaborated_data/HMP2_sample_data')
...
fread('results/ML/mbx__XGB_optimized_shapi.csv') %>%
  rowwise() %>% 
  mutate(wt = sum(nonIBD, UC, CD)) %>%
  ungroup %>% 
  top_n(n = 20) %>% 
